...
for(const relative of required){
  if(!existsSync(join(targetRoot,relative))){
    console.error(`V46A installer stopped: ${targetRoot} is not the expected V45 repository. Missing ${relative}.`);
    process.exit(1);
  }
}
...
if(!/visualref/i.test(packageJson.name||"")&&!/VisualRef/i.test(packageJson.description||"")){
  console.error("V46A installer stopped: package.json does not identify a VisualRef project.");
  process.exit(1);
}

const backupRoot=join(targetRoot,"_v45_backup_before_v46a");
...
await copyFile(join(sourceRoot,"docs/V46A_WORKFLOW_IMPLEMENTATION.md"),join(targetRoot,"docs/V46A_WORKFLOW_IMPLEMENTATION.md"));
await copyFile(join(sourceRoot,"docs/V46A_VALIDATION_REPORT.md"),join(targetRoot,"docs/V46A_VALIDATION_REPORT.md"));

for(const page of ["viewport.html","render.html","timeline.html"]){
  const path=join(targetRoot,page);
  let html=await readFile(path,"utf8");
  html=html.replace(/V45/g,"V46A").replace(/v45 · product vis/gi,"V46A · WORKFLOW PASS");
  ...
  if(existsSync(join(targetRoot,"css/v44-render-editor.css"))&&!html.includes("./css/v44-render-editor.css"))html=html.replace("</head>",'  <link rel="stylesheet" href="./css/v44-render-editor.css">\n</head>');
  ...
  html=html.replace(/data-memento-build="v45"/g,'data-memento-build="v46a"');
...
<head>
  ...
  <title>MEMENTO VisualRef · V46A</title>
...
packageJson.name="memento-visualref-v46a";
packageJson.version="46.1.0";
packageJson.description="MEMENTO VisualRef V46A — single-runtime Viewport → Render workflow stabilization and readable editor pass.";
packageJson.scripts={...(packageJson.scripts||{}),"check:v46a":"node --check src/v46/local-bootstrap.js && node --check src/v46/polish-controller.js"};
await writeFile(packagePath,JSON.stringify(packageJson,null,2)+"\n","utf8");

console.log("V46A workflow pass installed successfully.");
console.log(`Target: ${targetRoot}`);
console.log("Entry page: viewport.html");
console.log("Backup: _v45_backup_before_v46a/");
console.log("Run: npm run check && npm run check:v46a");
