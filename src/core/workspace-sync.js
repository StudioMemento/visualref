...
export class WorkspaceSync{
  constructor(){
    ...
    this.channel="BroadcastChannel" in globalThis?new BroadcastChannel("memento-visualref-v43"):null;
    if(this.channel)this.channel.onmessage=event=>this.receive(event.data);
    addEventListener("storage",event=>{if(event.key==="memento-v43-sync"&&event.newValue)this.receive(safeParse(event.newValue,null));});
  }
  onState(listener){this.listener=listener;}
  broadcast(state){
    const message={type:"state",source:this.id,sentAt:Date.now(),state};
    this.channel?.postMessage(message);
    try{localStorage.setItem("memento-v43-sync",JSON.stringify(message));}catch{}
  }
  receive(message){
