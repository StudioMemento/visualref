import {AXIS_MAP,DEFAULT_CORRECTION,DEFAULT_TRANSFORM,correctionDefaultsForType,HERO_NORMALIZATION} from "../core/default-state.js";
...
export class ViewportWorkspace{
  ...
  async importFiles(kind,files){
    ...
    try{
      for(const file of files){
        if(kind!=="hdri"&&!file.name.toLowerCase().endsWith(".glb"))throw new Error("V43C accepts self-contained .glb files");
        ...
        const asset={id,type:kind,kind:"uploaded",name:file.name,status:"ready",source:"indexeddb",size:file.size,mime:file.type||"application/octet-stream",nodeId,normalization:kind==="hero"?"camera":kind==="environment"?"native":"scene",updatedAt:new Date().toISOString()};
        const node=nodeId&&kind==="prop"?{id:nodeId,name:`Prop · ${file.name}`,type:"prop",assetId:id,visible:true,locked:false,baseTransform:structuredClone(DEFAULT_TRANSFORM),correction:correctionDefaultsForType("prop"),helpers:{bounds:true,pivot:false}}:null;
        this.commands.dispatch("asset.register",{asset,node});if(oldId&&!oldId.endsWith("proxy")&&oldId!==id)await this.persistence.deleteAsset(oldId);this.toast?.(`${kind.toUpperCase()} READY · ${file.name}`);if(nodeId)setTimeout(()=>this.renderer.frameNode(nodeId,false),700);
      }
  ...
  renderOutliner(state){
    ...
    this.root.querySelector('[data-role="outliner-list"]').innerHTML=order.map(id=>{const node=state.scene.nodes[id];if(!node)return"";const asset=node.assetId&&state.assets.byId[node.assetId],source=asset?.source==="indexeddb"?"LOCAL":asset?.source==="builtin"?"BUILTIN":"SCENE",contract=asset?.normalization==="camera"?"CAMERA FIT":asset?.normalization==="native"?"NATIVE":asset?.normalization==="scene"?"SCENE FIT":"";return `<button class="scene-node ${state.ui.selectedNodeId===id?"active":""}" data-node="${id}"><i>${icons[node.type]||"◇"}</i><span><b>${esc(node.name)}</b><small>${source}${contract?` · ${contract}`:""}${node.visible===false?" · HIDDEN":""}</small></span><em>${node.locked?"LOCK":"●"}</em></button>`;}).join("");
  }
  renderInspector(state,node){
    const shot=activeShot(state),asset=node.assetId?state.assets.byId[node.assetId]:null,hero=node.type==="hero",environment=node.type==="environment",editable=["hero","environment","prop"].includes(node.type),metrics=this.renderer.getNodeMetrics?.(node.id);
    const scope=`<section class="section"><header class="section-header"><span>ENDPOINT SCOPE</span><b>SHOT LINKED</b></header><div class="section-body"><div class="scope-row"><button data-scope="start" class="${state.ui.editScope==="start"?"active":""}">START</button><button data-scope="both" class="${state.ui.editScope==="both"?"active":""}">BOTH</button><button data-scope="end" class="${state.ui.editScope==="end"?"active":""}">END</button></div></div></section>`;
    const contract=hero?"CAMERA NORMALIZED":environment?"NATIVE SCENE SPACE":node.type==="prop"?"SCENE NORMALIZED":"SCENE SERVICE";
    const sourceSize=metrics?.sourceSize?.map(value=>Number(value).toFixed(2)).join(" × ");
    let html=`${hero?scope:""}<section class="section"><header class="section-header"><span>SELECTED NODE</span><b>${esc(node.type.toUpperCase())}</b></header><div class="section-body"><div class="node-identity"><b>${esc(node.name)}</b><small>${asset?`${formatBytes(asset.size)} · ${esc(asset.source)} · ${contract}`:contract}</small></div>${editable?`<div class="viewport-toggle-row"><label><input data-toggle="visible" type="checkbox" ${node.visible!==false?"checked":""}>VISIBLE</label><button data-viewport-action="frame-selected">FRAME</button>${asset&&asset.source==="indexeddb"?`<button class="danger" data-remove-asset="${asset.id}" data-node-id="${node.id}">REMOVE</button>`:""}</div>`:""}</div></section>`;
    if(hero)html+=`<section class="section"><header class="section-header"><span>SHOT TRANSFORM</span><b>CAMERA-RELATIVE</b></header><div class="section-body"><div class="transform-note">Subject Size operates after camera normalization, so the same scale value produces a comparable frame presence across different Hero GLBs.</div><div class="axis-list compact">${HERO_AXES.map(id=>shotAxis(id,state,shot)).join("")}</div></div></section>`;
    else if(editable)html+=`<section class="section"><header class="section-header"><span>NODE TRANSFORM</span><b>${environment?"ENVIRONMENT SPACE":"SCENE BASE"}</b></header><div class="section-body">${environment?`<div class="transform-note">Environment assets preserve native scale and origin. Position, rotation and scale remain fully editable and are never normalized to the Shot Camera.</div>`:""}${transformControls(node.baseTransform,"base",{wide:environment})}</div></section>`;
    if(hero)html+=`<section class="section correction-section"><header class="section-header"><span>HERO NORMALIZATION</span><b>CAMERA FIT</b></header><div class="section-body"><div class="transform-note">Locked camera contract · ${Math.round((node.correction?.targetCoverage??HERO_NORMALIZATION.targetCoverage)*100)}% reference coverage at ${HERO_NORMALIZATION.referenceFov}° / ${HERO_NORMALIZATION.referenceDistance}. ${sourceSize?`Source bounds: ${sourceSize}.`:""}</div><div class="viewport-toggle-row"><label><input type="checkbox" checked disabled>CAMERA NORMALIZED</label><label><input data-toggle="autoGround" type="checkbox" ${node.correction?.autoGround!==false?"checked":""}>AUTO GROUND</label><button data-reset-correction="${node.id}">RESET</button></div>${correctionControls(node.correction,{showScale:true})}</div></section>`;
    else if(environment)html+=`<section class="section correction-section"><header class="section-header"><span>ENVIRONMENT PIVOT</span><b>NATIVE SPACE</b></header><div class="section-body"><div class="transform-note">No camera normalization is applied. Pivot, orientation, correction scale and ground offset are non-destructive and stored with the project. ${sourceSize?`Native bounds: ${sourceSize}.`:""}</div><div class="viewport-toggle-row"><label><input data-toggle="autoGround" type="checkbox" ${node.correction?.autoGround===true?"checked":""}>AUTO GROUND</label><button data-reset-correction="${node.id}">RESET</button></div>${correctionControls(node.correction,{showScale:true,wide:true})}</div></section>`;
    else if(editable)html+=`<section class="section correction-section"><header class="section-header"><span>IMPORT CORRECTION</span><b>SCENE FIT</b></header><div class="section-body"><div class="viewport-toggle-row"><label><input data-toggle="autoNormalize" type="checkbox" ${node.correction?.autoNormalize!==false?"checked":""}>AUTO NORMALIZE</label><label><input data-toggle="autoGround" type="checkbox" ${node.correction?.autoGround!==false?"checked":""}>AUTO GROUND</label><button data-reset-correction="${node.id}">RESET</button></div>${correctionControls(node.correction,{showScale:true})}</div></section>`;
...
function transformControls(transform,prefix,{wide=false}={}){return `<div class="vector-grid">${["position","rotation","scale"].map(part=>`<fieldset><legend>${part.toUpperCase()}</legend>${[0,1,2].map((index)=>{const min=part==="scale"?(wide?.0001:.01):part==="rotation"?-180:(wide?-10000:-30),max=part==="scale"?(wide?1000:20):part==="rotation"?180:(wide?10000:30),step=part==="rotation"?1:part==="scale"?(wide?.001:.01):(wide?.1:.01);return simpleControl(`${prefix}:${part}:${index}`,"XYZ"[index],part==="rotation"?deg(transform[part][index]):transform[part][index],min,max,step);}).join("")}</fieldset>`).join("")}</div>`;}
function correctionControls(correction={},options={}){const c={...DEFAULT_CORRECTION,...correction},wide=Boolean(options.wide);return `<div class="vector-grid">${["pivot","rotation","scale"].map(part=>`<fieldset><legend>${part.toUpperCase()}</legend>${[0,1,2].map(index=>{const min=part==="scale"?(wide?.0001:.01):part==="rotation"?-180:(wide?-10000:-20),max=part==="scale"?(wide?1000:20):part==="rotation"?180:(wide?10000:20),step=part==="rotation"?1:part==="scale"?(wide?.001:.01):(wide?.1:.01);return simpleControl(`correction:${part}:${index}`,"XYZ"[index],part==="rotation"?deg(c[part][index]):c[part][index],min,max,step);}).join("")}</fieldset>`).join("")}</div>${simpleControl("correction:groundOffset","GROUND OFFSET",c.groundOffset,wide?-10000:-10,wide?10000:10,wide?.1:.01)}`;}
