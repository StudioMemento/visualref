...
export class AppShell{
  ...
  build(){
    ...
    this.root.innerHTML=`<div class="app-shell">
      <header class="app-header"><div class="brand-zone">${LOGO}</div><nav class="mode-zone">${modeLinks}</nav><div class="action-zone">
        ...
        <button class="project-button" data-shell-action="project"><span data-role="project-name">MEMENTO V43B.1</span></button>
  ...
  renderDialog(state){
    ...
    this.dialogHost.innerHTML=`<div class="project-dialog" data-project-close><section class="project-card" onclick="event.stopPropagation()"><header><div><small>PROJECT STATE</small><b>V43B.1 TIMELINE MONITOR</b></div><button data-project-close>×</button></header><div class="project-body"><label class="project-field"><span>PROJECT NAME</span><input data-project-name value="${escapeHtml(state.meta.name)}"></label><div class="project-meta"><span>AUTOSAVE</span><b>PROJECT + BINARY ASSETS</b><span>SCHEMA</span><b>${state.schema.name} · ${state.schema.version}</b><span>WORKSPACE</span><b>${state.ui.activeWorkspace.toUpperCase()}</b></div><div class="project-actions"><button data-project-save-name>SAVE NAME</button><button class="danger" data-project-reset>RESET PROJECT</button></div></div></section></div>`;
